...
export class EtablissementService {
  ...
  getAddressFromCoordinates(longitude: any, latitude: any) {
    return this.http.get<any>(`${this.baseUrl}getAddress?longitude=${longitude}&latitude=${latitude}`);
  }
  getMedecinsByEtab(idEtab: number): Observable<any> {
    return this.http.get<any>(`${this.url}etabMed/${idEtab}`);
  }

  getInfirmiersByEtab(idEtab: number): Observable<any> {
    return this.http.get<any>(`${this.url}etabinf/${idEtab}`);
  }
  getPatientsByEtab(idMed: number): Observable<any> {
    return this.http.get<any>(`${this.url}etabpat/${idMed}`);
  }
}
